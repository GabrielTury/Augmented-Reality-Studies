...
namespace AnythingWorld.Editor
{
    ...
    public class AnythingCreatorEditor : AnythingEditor
    {
        ...
        private AnythingWorld.Utilities.Data.SearchResult lastResult;
        ...
        private Texture2D textureThumb;
        ...
        public static Dictionary<DefaultBehaviourType, Type> DefaultBehaviourDictionary
        {
            get
            {
                ...
                if (TransformSettings.RiggedAnimalScript != null) temp.Add(DefaultBehaviourType.WalkingAnimal, TransformSettings.RiggedAnimalScript?.GetClass());
                if (TransformSettings.GroundVehicleScript != null) temp.Add(DefaultBehaviourType.WheeledVehicle, TransformSettings.GroundVehicleScript?.GetClass());
                if (TransformSettings.FlyingVehicleScript != null) temp.Add(DefaultBehaviourType.FlyingVehicle, TransformSettings.FlyingVehicleScript?.GetClass());
                if (TransformSettings.StaticBehaviourScript != null) temp.Add(DefaultBehaviourType.Static, TransformSettings.StaticBehaviourScript?.GetClass());
                if (TransformSettings.FlyingAnimalScript != null) temp.Add(DefaultBehaviourType.FlyingAnimal, TransformSettings.FlyingAnimalScript?.GetClass());
                if (TransformSettings.SwimmingAnimalScript != null) temp.Add(DefaultBehaviourType.SwimmingAnimal, TransformSettings.SwimmingAnimalScript?.GetClass());
                return temp;
            }
        }
        #region Filters
        ...
        protected void DrawSettingsIcons(Rect settingsRect)
        {
            ...
            var makeOptionsContent = new GUIContent("Make Options", transformWindowOpen ? StateTransformIcon.activeTexture : StateTransformIcon.inactiveTexture);
            var makeOptionsRect = new Rect(settingsRect.x, settingsRect.y, Mathf.Max(settingsRect.width / 4, 125f), settingsRect.height);
        ...
        protected void MakeResult(SearchResult result, bool forceSerialize = false)
        {
            ...
            {
                RequestParameter.Position(TransformSettings.PositionField),
                RequestParameter.Rotation(TransformSettings.RotationField),
                RequestParameter.ScaleMultiplier(TransformSettings.ScaleField),
                RequestParameter.Parent(objectParent),

                RequestParameter.PlaceOnGrid(TransformSettings.PlaceOnGrid),
                RequestParameter.PlaceOnGround(TransformSettings.PlaceOnGround),
                RequestParameter.UseGridArea(TransformSettings.GridAreaEnabled),

                RequestParameter.AddRigidbody(TransformSettings.AddRigidbody),
                RequestParameter.AddCollider(TransformSettings.AddCollider),
                RequestParameter.SetDefaultBehaviourPreset(DefaultBehavioursUtility.CreateNewTemporaryInstance(DefaultBehaviourDictionary)),
                RequestParameter.IsAnimated(TransformSettings.AnimateModel),
                RequestParameter.SerializeAsset(TransformSettings.SerializeAsset || forceSerialize)
            };
            GameObject gameObject;
            //if the result is processed, make the object with the json
            if (result.IsProcessedResult)
            {
                gameObject = AnythingMaker.Make(result.json, inputParams.ToArray());
            }
            else
            {
                gameObject = AnythingMaker.Make(result.data.name, inputParams.ToArray());
            }
        ...
        protected GameObject MakeResult(SearchResult result, Vector3 position, bool forceSerialize = false)
        {
            ...
            var inputParams = new List<RequestParameterOption>
            {
                RequestParameter.Position(position),
                RequestParameter.Rotation(TransformSettings.RotationField),
                RequestParameter.ScaleMultiplier(TransformSettings.ScaleField),
                RequestParameter.Parent(objectParent),
                //in this case aways not place on grid
                RequestParameter.PlaceOnGrid(false),
                RequestParameter.UseGridArea(false),
                RequestParameter.PlaceOnGround(TransformSettings.PlaceOnGround),
            ...
            if (CheckCollision(gameObject))
            {
                ...
                if (TransformSettings.PlaceOnGround)
                {
                    ...
                    if (Physics.Raycast(gameObject.transform.position + Vector3.up * 100, Vector3.down, out hit, 200f))
                    {
                        ...
                        gameObject.transform.position = new Vector3(gameObject.transform.position.x, hit.point.y + gameObject.GetComponentInChildren<Renderer>().bounds.extents.y, gameObject.transform.position.z);
                    }
                }
            }
        }
        ...
        IEnumerator WaitAndPutOnGround(GameObject ob)
        {
            while (ob.transform.childCount < 1)
            {
                yield return new WaitForSeconds(0.5f);
            ...
            if (Physics.Raycast(ob.transform.position, Vector3.down, out hit, 100f))
            {
                ...
                //put the object in the ground respecting bounding box
                ob.transform.localPosition = new Vector3(ob.transform.localPosition.x, hit.point.y + ob.GetComponentInChildren<Renderer>().bounds.extents.y, ob.transform.localPosition.z);
        ...
        protected void DrawCardVoteButton(SearchResult result, ref Rect infoBackdropRect, float voteIconSizeX, float voteIconSizeY, float infoPaddingX, float infoPaddingY, out Rect voteRect)
        {
            ...
            if (GUI.Button(voteRect, new GUIContent((string)null, "Like Model"), new GUIStyle(IconStyle) { normal = SetStyleState(result.data.userVote == "upvote" ? StateHeartIcon.activeTexture : StateHeartIcon.inactiveTexture), hover = SetStyleState(StateHeartIcon.hoverTexture) }))
            {
        ...
        protected void DrawCardListIcon(SearchResult result, Rect infoRect, float iconSizeX, float iconSizeY, float infoPaddingX)
        {
            ...
            if (GUI.Button(listRect, new GUIContent((string)null, "Add to Collection"), new GUIStyle(IconStyle) { normal = SetStyleState(StateCollectionIcon.activeTexture), hover = SetStyleState(StateCollectionIcon.hoverTexture) }))
            {
                CollectionProcessor.GetCollectionNames(CollectionReceived, EditorNetworkErrorHandler.HandleError, this);
                void CollectionReceived(string[] results)
                {
                    existingCollections = results;
                    AnythingSubwindow.OpenWindow("Add to Collection", new Vector2(Mathf.Max(position.width - 80, 450), 200), DrawCollectionWindow, position, result);
                }
            }
        }

        protected void DrawCardInfoIcon(SearchResult result, ref Rect infoBackdropRect, float iconSizeX, float iconSizeY, float infoPaddingX, float infoPaddingY, out Rect infoRect)
        {
            infoRect = new Rect(infoBackdropRect.xMax - infoPaddingX - iconSizeX, infoBackdropRect.yMax - infoPaddingY - iconSizeY, iconSizeX, iconSizeY);
            if (GUI.Button(infoRect, new GUIContent((string)null, "More Details"), new GUIStyle(IconStyle) { normal = SetStyleState(StateInfoIcon.activeTexture), hover = SetStyleState(StateInfoIcon.hoverTexture) }))
            {
                selectedResult = result;
            }
        }

        protected void DrawCardAnimationStatusIcon(Rect thumbnailBackdropRect, float iconSizeX, float iconSizeY, float infoPaddingX, float infoPaddingY)
        {
            var animatedRect = new Rect(thumbnailBackdropRect.x + infoPaddingX, thumbnailBackdropRect.y + infoPaddingY, iconSizeX, iconSizeY);
        ...
        protected void DrawCardModelNameLabel(GUIContent modelName, ref Rect infoBackdropRect, float infoPaddingX, float infoPaddingY, out Rect modelNameLabelRect, float scaleMultiplier)
        {
            ...
            modelNameLabelRect = new Rect(modelNameXPos, infoBackdropRect.y + infoPaddingY, modelNameLabelWidth, modelNameSize.y);
            GUI.Label(modelNameLabelRect, modelName, modelNameStyle);
        }

        protected void DrawCardAuthorLabel(GUIContent authorName, ref Rect infoBackdropRect, float infoPaddingX, float infoPaddingY, Rect modelNameLabelRect, float scaleMultiplier)
        {
            ...
            var authorNameLabelRect = new Rect(authorNameXPos, modelNameLabelRect.yMax - infoPaddingY, authorNameLabelWidth, authorNameSize.y);
        ...
        protected void DrawCollectionWindow(AnythingEditor window, SearchResult result)
        {
            ...
            {
                CollectionProcessor.AddToCollection(MyWorldEditor.Instance.RefreshCollectionResults, result, existingCollections[existingCollectionIndex], EditorNetworkErrorHandler.HandleError, this);
                window.Close();
            }
            ...
            {
                CollectionProcessor.AddToCollection(MyWorldEditor.Instance.RefreshCollectionResults, result, collectionSearchTerm, EditorNetworkErrorHandler.HandleError, this);
        ...
        protected void DrawReportWindow(AnythingEditor window, SearchResult result)
        {
            ...
            if (!reportSent)
            {
                ...
                {
                    ReportProcessor.SendReport(DrawPostReportSent, result, reportReason, EditorNetworkErrorHandler.HandleError, this);
                }
            }
