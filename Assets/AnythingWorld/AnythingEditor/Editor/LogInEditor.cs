...
namespace AnythingWorld.Editor
{
    public class LogInEditor : AnythingCreatorEditor
    {
        ...
        [MenuItem("Anything World/Log In \u2044 Log Out", false, 2)]
        ...
        private void DrawLoginDetails(float marginSize)
        {
            ...
            DrawHeaderLabel("Login with API Key");
            GUILayout.Space(5f);
        ...
        private void DrawHeaderLabel(string label)
        {
            ...
            var labelStyle = new GUIStyle(HeaderLabelStyle) { normal = SetStyleState(EditorGUIUtility.isProSkin ? Color.white : Color.black), hover = SetStyleState(EditorGUIUtility.isProSkin ? Color.white : Color.black), fontSize = 20, font = GetPoppinsFont(PoppinsStyle.SemiBold) };
            var labelSize = labelStyle.CalcSize(labelContent);
            var headerRect = GUILayoutUtility.GetRect(position.width, labelSize.y);
            var labelRect = new Rect(headerRect.center.x - (labelSize.x / 2), headerRect.y, labelSize.x, labelSize.y);
